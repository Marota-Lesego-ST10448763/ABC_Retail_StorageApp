...
// Azure Function class for handling product operations
public class ProductsFunctions
{
    private readonly string _conn;    // Storage connection string
    private readonly string _table;   // Table name for products
    private readonly string _images;  // Blob container name for product images
    ...
    // POST /api/products — create a new product (supports multipart or JSON)
        ...
        [HttpTrigger(AuthorizationLevel.Anonymous, "put", Route = "products/{id}")] HttpRequestData req, string id)
    {
        ...
        try
        {
            ...
            // Handle multipart/form-data updates
            if (contentType.StartsWith("multipart/form-data", StringComparison.OrdinalIgnoreCase))
            {
                var form = await MultipartHelper.ParseAsync(req.Body, contentType);
